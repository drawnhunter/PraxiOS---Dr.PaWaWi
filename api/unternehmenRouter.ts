...
export const unternehmenRouter = createRouter({
  ...
  kennwerte: rechtQuery("abrechnung").query(async () => {
    return getDb()
      .select({
        k: companyKennwerte,
  ...
  kennwertAnlegen: rechtQuery("abrechnung").input(kennwertInput).mutation(async ({ input }) => {
  ...
  kennwertAktualisieren: rechtQuery("abrechnung")
  ...
  kennwertLoeschen: rechtQuery("abrechnung")
  ...
  belegAuswahl: rechtQuery("abrechnung").query(async () => {
    return getDb()
      .select({
        id: postEingang.id,
