...
export const lagerRouter = createRouter({
  ...
  bestand: rechtQuery("lager").query(async () => {
    const db = getDb();
  ...
  vergleich: rechtQuery("lager").query(async () => {
    const db = getDb();
