...
export const bankTransaktionenRouter = createRouter({
  ...
  importierenPdf: rechtQuery("abrechnung")
    .input(z.object({
  ...
  zuordnungenBuchen: rechtQuery("abrechnung")
    .input(
      z.object({
  ...
  kontenUebersicht: rechtQuery("abrechnung").query(async () => {
    const db = getDb();
  ...
  offeneZiele: rechtQuery("abrechnung")
  ...
  zuordnen: rechtQuery("abrechnung")
  ...
  loeschen: rechtQuery("abrechnung")
    .input(z.object({ transaktionId: z.number() }))
    .mutation(async ({ input }) => {
      const db = getDb();
      const t = await db.query.bankTransaktionen.findFirst({ where: eq(bankTransaktionen.id, input.transaktionId) });
  ...
  importLoeschen: rechtQuery("abrechnung")
  ...
  offeneFuerRechnung: rechtQuery("abrechnung")
  ...
  einzelTx: rechtQuery("abrechnung")
    .input(z.object({ transaktionId: z.number() }))
    .query(async ({ input }) => {
  ...
  kontoauszugPdf: rechtQuery("abrechnung")
    .input(
      z.object({
        bankAccountId: z.number(),
        von: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullish(),
  ...
  fuerRechnung: rechtQuery("abrechnung")
    .input(z.object({ invoiceId: z.number() }))
    .query(async ({ input }) => {
      const db = getDb();
      return db
