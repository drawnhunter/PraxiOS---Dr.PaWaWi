...
export const posteingangRouter = createRouter({
  ...
  buchen: rechtQuery("abrechnung").input(z.object({ id: z.number().int() })).mutation(async ({ input }) => {
    const db = getDb();
  ...
  loeschen: rechtQuery("abrechnung").input(z.object({ id: z.number().int() })).mutation(async ({ input }) => {
    const zeile = await getDb().query.postEingang.findFirst({ where: eq(postEingang.id, input.id) });
    if (!zeile) throw new Error("Dokument nicht gefunden.");
  ...
  ocrAnalysieren: rechtQuery("abrechnung").input(z.object({ id: z.number().int() })).mutation(async ({ input }) => {
    const zeile = await getDb().query.postEingang.findFirst({ where: eq(postEingang.id, input.id) });
    if (!zeile) throw new Error("Dokument nicht gefunden.");
  ...
  zahlungsziele: rechtQuery("abrechnung").query(async () => {
    const db = getDb();
