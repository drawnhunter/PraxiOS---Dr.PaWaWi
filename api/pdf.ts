...
export function renderBelegPdf(beleg: PdfBeleg, design?: PdfDesign): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    beleg.items.forEach((it, idx) => {
      ...
      // über die ganze Breite, ohne Nummer und Zahlen (z. B. „1. Ärztliche
      // Leistungen (GOÄ)" / „2. Auslagen gemäß § 10 GOÄ")
      if (mitPreisen && Number(it.menge) === 0 && Number(it.einzelpreis) === 0) {
        const abschnittH = Math.max(
          doc.font(bold).heightOfString(it.bezeichnung, { width: CONTENT_W - 8 }) + 10,
          K ? 16 : 19,
        );
        if (y + abschnittH > unterkante) {
          doc.addPage();
          y = zeichneTabellenkopf(MARGIN);
          doc.font(regular).fontSize(basisSchrift);
        }
        doc.rect(MARGIN, y, CONTENT_W, abschnittH).fillColor("#e9e9e9").fill();
        doc.font(bold).fontSize(basisSchrift).fillColor(DARK);
        doc.text(it.bezeichnung, MARGIN + 4, y + 5, { width: CONTENT_W - 8 });
        y += abschnittH;
        return;
      }
      posNr += 1;
      ...
      doc.text(String(posNr), colX[0] + 4, y + 5, { width: COLS[0].w - 8 });
