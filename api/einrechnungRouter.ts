...
export const einrechnungRouter = createRouter({
  ...
  analysierenPdf: rechtQuery("abrechnung")
  ...
  analysieren: rechtQuery("abrechnung").input(xmlInput).mutation(async ({ input }) => {
  ...
  buchen: rechtQuery("abrechnung").input(xmlInput).mutation(async ({ input }) => {
  ...
  xml: rechtQuery("abrechnung").input(z.object({ id: z.number() })).query(async ({ input }) => {
    const r = await getDb().query.incomingInvoices.findFirst({
      where: eq(incomingInvoices.id, input.id),
    });
