...
// Provider-Presets (PaWaWi 1.15.0, Bus #60): Host/Port vorbefüllen, Hinweis geben.
const PRESETS: Record<string, { label: string; werte: Partial<KontoForm>; hinweis: string }> = {
  gmail: {
    label: "Gmail",
    werte: { host: "imap.gmail.com", port: 993, tls: true, smtpHost: "smtp.gmail.com", smtpPort: 465 },
    hinweis:
      "Gmail braucht ein App-Passwort (NICHT das normale Passwort): Google-Konto → Sicherheit → Bestätigung in zwei Schritten → App-Passwörter. Das App-Passwort unten bei Passwort UND bei SMTP-Passwort eintragen; Benutzername = vollständige Gmail-Adresse.",
  },
  gmx: {
    label: "GMX",
    werte: { host: "imap.gmx.net", port: 993, tls: true, smtpHost: "mail.gmx.net", smtpPort: 587 },
    hinweis: "GMX: normales Passwort; Benutzername = vollständige GMX-Adresse.",
  },
  webde: {
    label: "WEB.DE",
    werte: { host: "imap.web.de", port: 993, tls: true, smtpHost: "smtp.web.de", smtpPort: 587 },
    hinweis: "WEB.DE: normales Passwort; Benutzername = vollständige WEB.DE-Adresse.",
  },
};
    ...
    <section className="rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent className="max-w-lg">
          ...
          {!form.id && (
            <div className="mb-3">
              <Label className="text-xs">Provider-Vorlage (füllt Host/Port)</Label>
              <div className="mt-1 flex flex-wrap gap-1.5">
                {Object.entries(PRESETS).map(([key, p]) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setForm({ ...form, ...p.werte, benutzer: form.benutzer, smtpBenutzer: form.smtpBenutzer || form.benutzer });
                      setPresetHinweis(p.hinweis);
                    }}
              ...
              {presetHinweis && (
                <p className="mt-2 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">{presetHinweis}</p>
              )}
            </div>
          )}
